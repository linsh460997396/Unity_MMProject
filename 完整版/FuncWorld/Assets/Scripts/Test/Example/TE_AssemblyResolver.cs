...
//             // 注意:这种方法可能不总是可靠的,特别是若主程序集不包含任何类型或者第一个类型的命名空间不是所需的
...
// 若资源文件位于项目的根目录下,则resourceName可能只需要文件名(包括扩展名).
// 若资源文件位于项目的某个文件夹中,则resourceName应包括文件夹路径(使用点.作为路径分隔符).