...
//MC插件升级至最新LTS
//默认禁用URP图像否则材质出问题,尝试材质升级显示了但依然有使用问题如旧镜头不清理留下的移动痕迹、无法使用已经旧的AB素材包(材质无法被识别需要重新打包)
//方法中的网络功能依然是旧版UNet(Obsolete黄字警告),建议升级后使用
namespace Uniblocks
{
    #region 枚举
    ...
    public class Engine : MonoBehaviour
    {
        ...
        /// </summary>
        public string lWorldName = "DefaultWorld";
        ...
        /// <summary>
        /// 衍生团块的最大正负垂直块索引,即团块自动创建时高度范围限值(若是3,表示高度Y最大是3个团块范围,若团块是16边长,那么Y最大高度48)
        /// </summary>
        public static int HeightRange;
        /// <summary>
        /// 从原点到生成团块的水平距离(以团块为单位),是团块自动创建时的距离限制(若是8,则始终在玩家周围保证有8范围的团块)
        /// </summary>
        public static int ChunkSpawnDistance;
        /// <summary>
        /// 一个团块的边长(以体素为单位),即团块自动创建时的正方形单边尺寸边长(若是16,则团块由16*16*16个体素块组成)
        /// </summary>
        public static int ChunkSideLength;
        /// <summary>
        /// 团块自动摧毁时的判断距离(若是3,则团块会在距离玩家ChunkSpawnDistance+3个团块距离时进行摧毁,摧毁时会存入档案以便玩家走过来时读取生成)
        /// </summary>
        ...
        /// <summary>
        /// [GUI界面输入]衍生团块的最大正负垂直块索引,即团块自动创建时高度范围限值(若是3,表示高度Y最大是3个团块范围,若团块是16边长,那么Y最大高度48)
        /// </summary>
        public int lHeightRange;
        /// <summary>
        /// [GUI界面输入]从原点到生成团块的水平距离(以团块为单位),是团块自动创建时的距离限制(若是8,则始终在玩家周围保证有8范围的团块)
        /// </summary>
        public int lChunkSpawnDistance;
        /// <summary>
        /// [GUI界面输入]一个团块的边长(以体素为单位),即团块自动创建时的正方形单边尺寸边长(若是16,则团块由16*16*16个体素块组成)
        /// </summary>
        public int lChunkSideLength;
        /// <summary>
        /// [GUI界面输入]团块自动摧毁时的判断距离(若是3,则团块会在距离玩家ChunkSpawnDistance+3个团块距离时进行摧毁,摧毁时会存入档案以便玩家走过来时读取生成)
        /// </summary>
        ...
        /// <summary>
        /// 纹理表上纹理之间的填充,作为单个体素块纹理大小的一小部分.即每个纹理单元之间填充缝的大小(UV划区时向内缩进比例),填充以避免取到别的纹理单元
        /// </summary>
        ...
        /// <summary>
        /// [GUI界面输入]纹理表上纹理之间的填充,作为单个体素块纹理大小的一小部分.即每个纹理单元之间填充缝的大小(UV划区时向内缩进比例),填充以避免取到别的纹理单元
        /// </summary>
        ...
        /// <summary>
        /// 目标(预期)帧率,并非实际,若计时器记录每帧处理用时超过它,则可以将动作放在下一帧继续步进,防止卡在这一帧(让外围团块慢慢生成),所以这个值并非期望越高越好而是应尽量贴近实际.
        /// </summary>
        ...
        /// <summary>
        /// [GUI界面输入]目标(预期)帧率,并非实际,若计时器记录每帧处理用时超过它,则可以将动作放在下一帧继续步进,防止卡在这一帧(让外围团块慢慢生成),所以这个值并非期望越高越好而是应尽量贴近实际.
        /// </summary>
        ...
        /// <summary>
        /// 发送镜头注视事件(若为true, CameraEventsSender组件将把事件发送到主摄像机视场中心指着的体素块)
        /// </summary>
        public static bool SendCameraLookEvents;
        /// <summary>
        /// 发送鼠标指针事件(若为true, CameraEventsSender组件将将把事件发送到当前鼠标光标指着的体素块)
        /// </summary>
        ...
        /// 在多人游戏中,通常需要将物体的位置同步到其他客户端.轨道位置是指物体沿着一条路径或轨道移动时所处的位置.若将MultiplayerTrackPosition字段设置为true,则表示该物体的位置将在网络上进行同步,且每个客户端都将跟踪该物体的轨道位置.
        /// 若将MultiplayerTrackPosition字段设置为false,则表示该物体的位置不会在网络上进行同步,而客户端将不会跟踪其轨道位置.
        /// 在具有大量移动物体的多人游戏中,使用MultiplayerTrackPosition字段可以减少网络通信量并提高性能.例如,若某个物体在场景中静止不动,则将其MultiplayerTrackPosition字段设置为false可以避免不必要的网络同步.
        /// </summary>
        ...
        /// <summary>
        /// [GUI界面输入]发送镜头注视事件(若为true, CameraEventsSender组件将把事件发送到主摄像机视场中心指着的体素块)
        /// </summary>
        public bool lSendCameraLookEvents;
        /// <summary>
        /// [GUI界面输入]发送鼠标指针事件(若为true, CameraEventsSender组件将将把事件发送到当前鼠标光标指着的体素块)
        /// </summary>
        ...
        /// <summary>
        /// 允许团块超时(若Engine.chunkTimeout>0这个变量会自动设置为true)
        /// </summary>
        ...
        public void Awake()
        {
            ...
            {
                HeightRange = 0;
                Debug.LogWarning("Uniblocks: Chunk height range can'transform be a negative number! Setting chunk height range to 0." +
            ...
            if (MaxChunkDataRequests < 0)
            {
                ...
                Debug.LogWarning("Uniblocks: Max chunk data requests can'transform be a negative number! Setting max chunk data requests to 0." +
        ...
        /// “ignoreTransparent”为true时光线投射将穿透透明或半透明的体素块,若没有击中则返回null.注意:若碰撞体生成被禁用,此函数将不起作用.
        /// </summary>
        ...
        { // a raycast which returns the index of the hit voxel and the gameobject of the hit chunk
            ...
            if (Physics.Raycast(origin, direction, out hit, range))
            {
                ...
                { // check if we're actually hitting a chunk.检查我们是否真的击中了团块
                    ...
                    { // if we hit a mesh container instead of a chunk.若我们击中的是网状容器而不是团块(判断依据是网状容器拥有大块扩展组件),注意网格容器是团块大小的,虽是团块的子对象但它不是体素块)
                    ...
                    { // punch through transparent voxels by raycasting again when a transparent voxel is hit.当一个透明体素被击中时,再次通过光线投射穿透透明体素
                        ...
                        //若命中的体素类型的VTransparency属性!=实心,说明是透明或半透明
        ...
        /// “ignoreTransparent”为true时光线投射将穿透透明或半透明的体素块.若没有击中任何块,则返回null.注意:若碰撞体生成被禁用,此函数将不起作用.
        /// </summary>
        ...
        //使用纹理偏移点获取预制图片上的纹理:左下角精灵是(0,0),而(1,0)表示右边精灵,每个精灵的大小由Uniblocks引擎设置中Texture unit决定
        ...
        public static Vector2 GetTextureOffset(ushort voxel, Facing facing)
        {
            ...
            { // if this voxel isn'transform using custom side textures, return the Up texture.若这个体素没有使用自定义单面纹理,直接返回体素块朝上的那面纹理点(6个面共用)
