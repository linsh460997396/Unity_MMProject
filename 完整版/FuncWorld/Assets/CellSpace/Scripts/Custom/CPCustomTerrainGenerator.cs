    ...
    public class CPCustomTerrainGenerator : CPTerrainGenerator
    {
        ...
        public void Load3DMap()
        {
            ...
            {//刷小地图
                ...
                {
                    i++; //增加计数
                    if (currentX + 1 <= CPEngine.chunkSideLength && currentZ + 1 <= CPEngine.chunkSideLength) //检查团块空间边长是否够刷单元
                    {
                    ...
                    {
                        setErr = true;
                        Debug.LogError($"ErrorLoad3DMap: mapID={mapID}, currentX={currentX + 1}, currentZ={currentZ + 1} exceed chunk side length.");
                    }
                    currentX++;
                    //若达到行宽,则换行
                    if (currentX >= width)
        ...
        public void LoadUniverse()
        {
            ...
            int y = height - 1;
            //Debug.Log($"LoadUniverse: chunkx={chunkx}, chunky={chunky}, chunkz={chunkz}, mapID={mapID}, height={height}, y={y}");
