...
namespace CellSpace
{
    ...
    public class CellChunkManager : MonoBehaviour
    {
        ...
        /// </summary>
        public static List<CellChunk> ChunkUpdateQueue; // stores chunks ordered by update priority. Processed in the ProcessChunkQueue loop
        /// <summary>
        /// 团块销毁列表(存放在SpawnChunks结束时销毁的chunk)
        /// </summary>
        public static List<CellChunk> ChunksToDestroy; // chunks to be destroyed at the end of SpawnChunks
        /// <summary>
        ...
        /// 最后请求的索引.
        /// </summary>
        private static CPIndex LastRequest;
        /// <summary>
        /// 目标游戏周期 = 1f / CPEngine.targetFPS
        /// </summary>
        private static float targetFrameDuration;
        /// <summary>
        /// 帧计时器(统计用时)
        /// </summary>
        ...
        /// 团块创建队列
        /// </summary>
        private static int SpawnQueue;

        /// <summary>
        /// 初始化团块管理器:设定游戏周期(每帧秒数),建立团块组(字典),建立团块更新列表,建立帧计时器(负责记录处理用时),设置团块缩放比例为团块实例当前的缩放比例,设置附加网格碰撞器和触发器碰撞体的正确比例.
        /// </summary>
        public void Start()
        {
            //设定游戏周期(每帧秒数)
        ...
        /// 重启帧计时器(统计每帧处理用时)
        /// </summary>
        private static void ResetFrameStopwatch()
        {
            frameStopwatch.Stop();
        ...
        /// 处理团块队列循环(协程)的激活状态
        /// </summary>
        private static bool ProcessChunkQueueLoopActive;
        /// <summary>
        ...
        /// </summary>
        /// <returns></returns>
        private static IEnumerator ProcessChunkQueueLoop()
        ...
        /// <summary>
        /// 返回指定Index(pixelX, pixelY, z)的团块的游戏物体对象,若对象没有实例化则返回null.注意:这可能会有点慢,避免在一帧内用太多次.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="z"></param>
        ...
        /// <summary>
        /// 返回指定Index的团块的游戏物体对象,若对象没有实例化则返回null.注意:这可能会有点慢,避免在一帧内用太多次.
        /// </summary>
        ...
        /// <summary>
        /// 返回指定Index(pixelX, pixelY, z)的团块,若对象没有实例化则返回null.注意:这可能会有点慢,避免在一帧内用太多次.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="z"></param>
        ...
        /// <summary>
        /// 返回指定Index(pixelX, pixelY)的团块,若对象没有实例化则返回null.注意:这可能会有点慢,避免在一帧内用太多次.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 返回指定Index的团块,若对象没有实例化则返回null.注意:这可能会有点慢,避免在一帧内用太多次.
        /// </summary>
        /// <param name="index"></param>
        /// <returns></returns>
        ...
        /// 若已经在生成团块,标记以等上一轮结束后再生成,它使用最后请求的位置作为原点,创建队列计数为1,主动打断创建状态(SpawnChunks序列将被终止)并清除团块更新队列
        /// 注意:若你使用三个浮点数或Vector3作为参数的SpawnChunks,它将把参数作为世界位置并在生成团块之前将其转换为团块索引.
        /// 若你使用三个int或一个Index来做,则不会执行转换,会直接使用提供的索引位置.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="z"></param>
        ...
        /// 若已经在生成团块,标记以等上一轮结束后再生成,它使用最后请求的位置作为原点,创建队列计数为1,主动打断创建状态(SpawnChunks序列将被终止)并清除团块更新队列
        /// 注意:若你使用三个浮点数或Vector3作为参数的SpawnChunks,它将把参数作为世界位置并在生成团块之前将其转换为团块索引.
        /// 若你使用2个int或一个Index来做,则不会执行转换,会直接使用提供的索引位置.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        ...
        /// 注意:若你使用三个浮点数或Vector3作为参数的SpawnChunks,它将把参数作为世界位置并在生成团块之前将其转换为团块索引.
        /// 若你使用三个int或一个Index来做,则不会执行转换,会直接使用提供的索引位置.
        /// 世界位置转团块索引示范:currentPos = CPEngine.PositionToChunkIndex(chunkCreationIndicator.position);
        /// </summary>
        ...
        private void TrySpawnChunks(int x, int y, int z)
        {
            ...
            {
                if (Done == true)
                { //若当前没有生成团块动作,那就正常地生成它们
                    StartSpawnChunks(x, y, z);
                }
                else
                {
                    //若已经在生成团块,标记以等上一轮结束后再生成,它使用最后请求的位置作为原点
        ...
        /// 若已经在生成团块,标记以等上一轮结束后再生成,它使用最后请求的位置作为原点,创建队列计数为1,主动打断创建状态(SpawnChunks序列将被终止)并清除团块更新队列
        /// 注意:若你使用三个浮点数或Vector3作为参数的SpawnChunks,它将把参数作为世界位置并在生成团块之前将其转换为团块索引.
        /// 若你使用2个int或1个Index来做,则不会执行转换,会直接使用提供的索引位置.
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        ...
        {
            if (Done == true)
            { // if we're not spawning chunks at the moment, just spawn them normally.若当前没有生成团块动作,那就正常地生成它们
                        ...
                        for (var y = originY - currentLoop; y <= originY + currentLoop; y++)
                        {
                            for (var z = originZ - currentLoop; z <= originZ + currentLoop; z++)
                            {
                                ...
                                { //验证是否跳过高度范围之外的团块
                                    ...
                                    if (Mathf.Abs(originX - x) + Mathf.Abs(originZ - z) < range * 1.3f || (CPEngine.keepOneChunk && x - originX == 0 && y - originY == 0 && z - originZ == 0))
                ...
                { // iterate through all potential chunk indexes within range.遍历范围内所有可能的团块索引
                    for (var y = originY - currentLoop; y <= originY + currentLoop; y++)
                    {
                        ...
                        { // skip chunks outside of height range.这里跳过了高度范围之外的团块
                            //角落团块在XZ平面是根号2倍边长距离 < 1.3倍,所以会达不到那个索引,同时验证是否KeepOneChunk(至少保证原点插入1个团块)
                            if (Mathf.Abs(originX - x) + Mathf.Abs(originY - y) < range * 1.3f || (CPEngine.keepOneChunk && x - originX == 0 && y - originY == 0))
...
// 标记待销毁团块:
// 遍历当前存在的所有团块.
// 若某个团块与角色的距离超过range + CPEngine.chunkDespawnDistance(默认为11),则将其标记为待销毁.
// 若团块在垂直方向上超过范围,也将其标记为待销毁.
// 生成新团块:
// 通过三层嵌套循环,遍历以角色为中心、range为半径的立方体范围内的所有潜在团块索引.
// 检查每个潜在团块的Y坐标是否在允许的高度范围内.
// 跳过距离角色过远(超过range * 1.3f)的角落团块.
// 若更新队列不为空,则暂停循环并处理队列中的团块.
...
// 对于每个团块,检查其高度是否在指定的范围内,若是,则根据一定的条件生成新的团块或更新已有的团块.
// 在生成或更新团块的过程中,会根据一定的条件进行一些操作,如处理团块队列、实例化邻近团块、添加团块到队列等.
// 会检查是否需要在每帧结束前暂停生成,以控制生成的速率.若生成过程中需要停止生成,则会结束生成序列.