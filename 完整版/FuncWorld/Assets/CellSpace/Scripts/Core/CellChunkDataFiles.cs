...
namespace CellSpace
{
    ...
    public class CellChunkDataFiles : MonoBehaviour
    {
        ...
        /// </summary>
        /// <returns>若团块的单元数据为空则返回false,反之加载有效返回true</returns>
                ...
                { // this loop will stop once the CellData array has been populated completely. Iterates once per count-data block.
                  // 这个循环将在CellData数组被完全填充后停止,每个count-data单元仅迭代一次.
                    ...
                    while (ii < currentCount)
                    {
                        ...
                        chunk.SetCellSimple(i, currentData);// write a single voxel for every currentCount.若团块内大量连续索引的单元是同一种,压缩时currentCount计数会较大,在此处产生循环处理
        ...
        { // returns the data of chunk in compressed string format
            ...
            { // for each voxel
                ...
                if (thisData != currentData) //若团块的单元种类跟当前应赋予的种类不同
                ...
                { // if the data is the same as the last data, simply add to the count.若数据与最后一个数据相同,只需添加计数
                    currentCount++;
                }

                if (i == length - 1)
                { // if this is the last iteration of the loop, close and write the current block.若这是循环的最后一次迭代,关闭并写入当前单元
                    writer.Write((char)currentCount);
                    writer.Write((char)currentData);
                }
        ...
        { // returns the chunk data (from memory or from file), or an empty string if data can'transform be found
            ...
            {
                //若TempChunkData字典存在index键,返回其对应的值(团块的单元数据)
                return TempChunkData[indexString];
            }

            // try to load from region, return empty if not found.尝试从区域加载,若未找到则返回空
        ...
        { // returns the 1d index of a chunk's data in the region file
            ...
            //若团块索引是负的,以数据原点进行纠正,方便后面计算数组的一维索引
            if (newIndex.x < 0) newIndex.x = -newIndex.x - 1;
            if (newIndex.y < 0) newIndex.y = -newIndex.y - 1;
            if (CPEngine.horizontalMode)
            {//2D横版模式
                //存储时,区域边长设定为10(存储10个团块)
                //flatIndex = newIndex.x;

                //存储时,区域边长设定为10(存储100个团块)
        ...
        /// <returns>加载区域数据并从文件返回该数据,若未找到区域文件则返回null</returns>
        ...
        /// </summary>
        /// <param name="regionIndex">区域索引</param>
        /// <returns>若区域文件存在且尚未加载,则将区域数据加载到内存中,如数据存在(且已加载)则返回true,否则返回false</returns>
        ...
        { // creates an empty region file
            int iMax;
            Directory.CreateDirectory(CPEngine.worldPath); //创建世界名称文件夹(若不存在时)
        ...
        { //writes all chunk data from memory to disk, and clears memory
            ...
            foreach (string chunkIndex in TempChunkData.Keys)
            {
                ...
                // check if region is loaded, and load it if it's not.检查区域是否已加载,若未加载则加载
        ...
        private static void WriteRegionFile(string regionIndex)
        {
            ...
            foreach (string chunk in regionData)
            {
                ...
                {
                    //若计数没有达到区域数据长度上限,在每个团块数据后面进行字符插值(用于读取时分割子字符串)
                    writer.Write((char)ushort.MaxValue);
                }
                count++;
            }
            writer.Flush(); //使用Flush来保证写入(防止程序异常崩溃等情况导致写入错误)
            writer.Close(); //其实内部会自动调用Flush(),直接写Close()即可
            //Flush()的话,后面继续给writer内存里加减东西,然后写入存档,就不关闭流也行,不然每次写一点就频繁创建写入摧毁不效率
            //存档间隔挺大其实摧毁流下次再创建也无所谓,关闭流主要是防止占着大量内存
        }
    }
}

//主动保存=存储当前实例团块信息
//切图时摧毁团块触发的自动保存,是MC框架将超过距离的空间团信息从内存转移到硬盘