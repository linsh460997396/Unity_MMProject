...
namespace Uniblocks
{
    ...
    public class ChunkDataFiles : MonoBehaviour
    {
        ...
        /// <summary>
        /// 尝试从文件加载团块的体素数据,若未找到数据则返回false.
        /// </summary>
        /// <returns>若团块的体素数据为空则返回false,反之加载有效返回true</returns>
        ...
        { // decompresses voxel data and loads it into the CellData array
            ...
            int i = 0;
            int length = chunk.GetDataLength(); // length of CellData array.体素数据数组的长度
                ...
                { // this loop will stop once the CellData array has been populated completely. Iterates once per count-data block.
                  // 这个循环将在VoxelData数组被完全填充后停止,每个count-data体素块仅迭代一次.
                    ...
                    while (ii < currentCount)
                    {
                        ...
                        chunk.SetVoxelSimple(i, currentData);// write a single voxel for every currentCount.若团块内大量连续索引的体素块是同一种,压缩时currentCount计数会较大,在此处产生循环处理
        ...
        { // returns the data of chunk in compressed string format
            ...
            int i = 0;
            int length = chunk.GetDataLength(); // length of CellData array.团块的体素数据数组长度
            ...
            { // for each voxel
                ...
                if (thisData != currentData) //若团块的体素块种类跟当前应赋予的种类不同
                ...
                if (i == length - 1)
                { // if this is the last iteration of the loop, close and write the current block.若这是循环的最后一次迭代,关闭并写入当前体素块
                    writer.Write((char)currentCount);
                    writer.Write((char)currentData);
                }
        ...
        { // returns the chunk data (from memory or from file), or an empty string if data can'transform be found
            ...
            {
                //若TempChunkData字典存在index键,返回其对应的值(团块的体素数据)
        ...
        { // creates an empty region file
            ...
            Directory.CreateDirectory(Engine.WorldPath); //创建世界名称文件夹(若不存在时)
