...
namespace Uniblocks
{
    ...
    public class Chunk : MonoBehaviour
    {
        ...
        /// 那么由(3,3,3)返回[63]的公式VoxelData[(z * squaredSideLength) + (pixelY * SideLength) + pixelX]=3*16+3*4+3=63,边长为任意时同理.
        /// </summary>
        public ushort[] VoxelData; // main voxel data array
        /// <summary>
        /// 团块索引(pixelX,pixelY,z),这与它在世界上的位置直接相关.团块的位置始终是ChunkIndex*Engine.chunkSideLength(比如对于团块来说其索引位置增1就实际经过默认16个体素块长度,很容易理解).
        /// </summary>
        ...
        { // chunk initialization (load/generate data, set position, etc.)
            ...
            // multiply by scale.若团块缩放比例不是默认的1.0,则实际位置要根据缩放情况进行修改
            ...
            else if (Engine.SaveVoxelData && TryLoadVoxelData() == true)
            {
                ...
                //尝试从磁盘加载体素数据,TryLoadCellData()这个动作在条件里已经完成
            }
            else
            {
                //不存在则生成新的体素数据
        ...
        { //Calls GenerateCellData() in the script assigned in the TerrainGenerator variable.
        ...
        /// <summary>
        /// 更改指定数组索引处的体素数据(即修改体素块的种类),函数采用平面1D数组索引作为参数而不是x,pixelY,z的3D空间坐标.
        /// </summary>
        ...
        /// <summary>
        /// 更改指定索引处的体素数据(即修改体素块的种类).若updateMesh为true,则对标记团块的网格进行更新.当团块索引超过团块边界时将改变相应团块中的体素数据(如当前已实例化).
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="z"></param>
        ...
        /// <summary>
        /// 返回指定数组索引处的体素数据(即修改体素块的种类),函数采用平面1D数组索引作为参数而不是x,pixelY,z的3D空间坐标.
        /// </summary>
        /// <param name="rawIndex">平面1D数组索引</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 保存团块的体素数据到内存(TempChunkData),内部调用了ChunkDataFiles类的SaveData方法,如当前是WebPlayer则本地化存储会取消(本函数无效)
        /// </summary>
        ...
        /// <summary>
        /// 返回给定方向上与给定团块索引(pixelX,pixelY,z)相邻的团块索引.例如(0,0,0,Direction.left)将返回(-1,0,0).
        /// </summary>
        /// <param name="x"></param>
        /// <param name="y"></param>
        /// <param name="z"></param>
        ...
        { // converts pixelX,pixelY,z direction into a specific index
        ...
        /// <summary>
        /// [NetWork][协程]请求体素数据:等待直到连接到服务器,然后发送这个团块体素数据的请求到服务器,若没有连接就重置计数器
        /// </summary>
        /// <returns></returns>
        ...
        { // waits until we're connected to a server and then sends a request for voxel data for this chunk to the server.
          // 等待直到连接到服务器,然后发送这个团块体素数据的请求到服务器
            ...
            Engine.UniblocksNetwork.GetComponent<NetworkView>().RPC("SendCellData", RPCMode.Server, Network.player, ChunkIndex.x, ChunkIndex.y, ChunkIndex.z);
        }
