...
namespace EmeraldAI.SoundDetection
{
    ...
    {      
        ...
        void CheckForSounds ()
        {
            ...
            //Add each target from LineOfSightTargets to CurrentTargetData, given that it hasn'transform already been added and it has the EmeraldComponent.PlayerTag.
        ...
        void ClearThreats ()
        {
            ...
            //Only remove AI that don'transform exist within the current LineOfSightTargets after the AI has reached its Unaware ThreatLevel. 
        ...
        public void InvokeReactionList (ReactionObject SentReactionObject, bool SentByAttractModifier = false)
        {
            ...
            //只有在AI不处于战斗状态时才允许调用反应,因为战斗逻辑是单独处理的
            ...
            //Ensure the AI is using its Starting WanderType.确保AI正在使用它的起始漫游类型
            EmeraldComponent.MovementComponent.ChangeWanderType((EmeraldMovement.WanderTypes)EmeraldMovement.StartingWanderingType);
        ...
        {
            EmeraldComponent.MovementComponent.ChangeWanderType(EmeraldMovement.WanderTypes.Stationary); //Changes the AI's Wander Type to Stationary so that its default wandering type doesn'transform interfere with this waypoint generation process.
        ...
        {
            if (EmeraldComponent.MovementComponent.RotateTowardsTarget) yield break; //Don'transform allow the AI to rotate towards the TargetPosition if it's already doing so
            ...
            yield return new WaitForSeconds(0.1f); //Add a delay so the changes above aren'transform missed
