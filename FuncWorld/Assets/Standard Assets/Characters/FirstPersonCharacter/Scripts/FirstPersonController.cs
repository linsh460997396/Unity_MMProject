...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            m_NextStep = m_StepCycle / 2f;//用于确定下一次晃动何时发生，设置为 m_StepCycle 的一半这可能是为了在某个时间点触发晃动效果，比如当前晃动周期的一半时，如果 m_StepCycle 是用来跟踪当前晃动周期的话，那么 m_NextStep 可能用于确定何时开始下一个晃动周期
        ...
        private void Update()
        {
            ...
            //检查m_Jump变量是否为false。如果是，它会使用CrossPlatformInputManager来检测玩家是否按下了“Jump”按钮。如果玩家按下了跳跃按钮，m_Jump会被设置为true
            if (!m_Jump)
            {
            ...
            { //检查角色是否刚刚从非着陆状态变为着陆状态。如果是，它会执行以下动作
            ...
            {
                //检查角色是否在空中（即非着陆状态）并且没有在跳跃，但之前是在着陆状态。如果是它会将m_MoveDir.y设置为0f，意味着角色在空中时不会进行垂直移动
                m_MoveDir.y = 0f;
            }
        ...
        // 如果检测到表面，将 desiredMove 投影到这个表面上，确保角色沿着这个表面移动。
        // 设置移动方向:

        // 根据投影后的 desiredMove 和玩家的移动速度 speed，设置 m_MoveDir（即角色实际的移动方向）。
        // 处理地面状态和跳跃:

        // 如果角色站立在地面上 (m_CharacterController.isGrounded 为真)：
        // 向下施加一个较小的力 (m_StickToGroundForce)，使角色紧贴地面。
        // 如果玩家按下跳跃键 (m_Jump 为真)，则增加向上的跳跃力，播放跳跃声音，并更新跳跃状态。
        // 如果角色不在地面上，则考虑重力影响，增加向下的力。
        // 移动角色并处理碰撞:
