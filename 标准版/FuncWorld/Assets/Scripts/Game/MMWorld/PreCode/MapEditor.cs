...
namespace MMWorld
{
    ...
    public class MapEditor : MonoBehaviour
    {
        ...
        /// 若勾选"仅读内置"则始终使用内部路径,否则有外部路径文件存在时将被自动使用.
        /// </summary>
            ...
            if (!colliderToggle.isOn)
            {
                ...
                if (startID != -1 && MapID >= 0 && MapID < 240)
                {
                    ...
                    {//如鼠标碰到有效单元(没出当前网格范围)
                        if (Input.GetMouseButtonDown(0))
                        {//左键则放置标记在单元位置
                            mapIndex = (int)mousePosition.x + gridWidth * (int)mousePosition.y;//若是拉多镇21宽度,鼠标点击第一格是0+21*0的索引[0]
        ...
        /// 若开启"仅读内置"则始终使用内部路径的碰撞文件,否则有外部路径文件存在时将被自动使用.
        /// </summary>
