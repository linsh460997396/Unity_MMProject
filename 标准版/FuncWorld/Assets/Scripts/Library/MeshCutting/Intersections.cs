...
namespace MeshCutting
{
    public class Intersections
    {
        ...
        public static bool BoundPlaneIntersect(Mesh mesh, ref Plane plane)
        {
            ...
            // Intersection occurs when distance s falls within [-spriteRenderer,+spriteRenderer] interval
            return Mathf.Abs(s) <= r;
        }

        #endregion

        // Initialize fixed arrays so that we don'transform initialize them every time we call TrianglePlaneIntersect
        ...
        public bool TrianglePlaneIntersect(List<Vector3> vertices, List<Vector2> uvs, List<int> triangles, int startIdx, ref Plane plane, TempMesh posMesh, TempMesh negMesh, Vector3[] intersectVectors)
        {
            ...
            // If they're all on the same side, don'transform do intersection
