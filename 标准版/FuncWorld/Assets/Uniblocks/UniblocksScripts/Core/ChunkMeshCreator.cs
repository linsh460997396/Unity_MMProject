...
namespace Uniblocks
{
    ...
    public class ChunkMeshCreator : MonoBehaviour
    {
        ...
        public void RebuildMesh()
        {
            ...
            //意思是对于Voxels中的每一个体素(voxel),检查它是否有任何一面是暴露在外的,若有就把这一面添加到体素网格数组(名为 Vertices 和 Faces的列表)中
                    ...
                    while (z < SideLength)
                    {
                        ...
                        { // don'transform render empty blocks.不渲染空块
                            ...
                            { // if cube.若是立方体
                                ...
                                //Transparency transparency = Engine.GetCellType (chunk.GetCellID(pixelX,pixelY,z)).VTransparency;
                                ...
                                // if no collider, create a trigger cube collider.若没有碰撞体,创建一个立方体碰撞体
                            ...
                            { // if not cube.若不是立方体
                                ...
                                { // if any adjacent voxel isn'transform opaque, we render the mesh.若任何相邻体素不是不透明的,自定义渲染网格
        ...
        { // returns true if a face should be spawned
            ...
            { // if the neighbor chunk is missing.若相邻团块是缺失状态
