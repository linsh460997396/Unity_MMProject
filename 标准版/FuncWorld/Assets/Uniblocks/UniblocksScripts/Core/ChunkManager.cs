...
namespace Uniblocks
{
    ...
    public class ChunkManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// 【服务器】用索引(pixelX,pixelY,z)生成单个团块的游戏对象并返回(索引点将作为团块在世界的位置),前提是该索引位置尚未实例化团块对象.本函数会禁用网格生成并启用超时(由服务器在多人游戏中使用),但若团块实例已存在则不会禁用网格生成和启用超时,仅简单地返回该对象.
        /// </summary>
        /// <param name="index">团块索引</param>
        /// <returns>返回团块的游戏物体对象</returns>
                    ...
                    for (var y = originY - currentLoop; y <= originY + currentLoop; y++)
                    {
                        for (var z = originZ - currentLoop; z <= originZ + currentLoop; z++)
                        {
                            ...
                            { // skip chunks outside of height range.这里跳过了高度范围之外的团块
                                if (Mathf.Abs(originX - x) + Mathf.Abs(originZ - z) < range * 1.3f) //角落团块在XZ平面是根号2倍边长距离<1.3倍,所以达不到那个索引
...
// 标记待销毁团块:
// 遍历当前存在的所有团块.
// 若某个团块与角色的距离超过range + Engine.chunkDespawnDistance(默认为11),则将其标记为待销毁.
