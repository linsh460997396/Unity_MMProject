...
namespace CellSpace
{
    ...
    public class CellChunkMeshCreator : MonoBehaviour
    {
        ...
        public void RebuildMesh()
        {
            ...
            //意思是对于Voxels中的每一个体素,检查它是否有任何一面是暴露在外的,若有就把这一面添加到体素网格数组(名为 Vertices 和 Faces的列表)中
            //Vertices和Faces是两个数组,分别表示三维空间中的顶点坐标和多边形的面信息
            //Vertices 数组存储了所有顶点的坐标,而 Faces 数组则存储了由这些顶点构成的多边形的面信息
            //若一个体素的某个面暴露在外,那么就会把该面的顶点坐标添加到 Vertices 数组中,并把该面的信息(即哪些顶点构成了这个面)添加到 Faces 数组中
                          ...
                          //获取体素数据(体素块的种类)对应的体素(类型)
                            ...
                            { // if cube.若不是自定义网格
                                ...
                                {//多维横版功能开启后对于上下左右4个侧面,应显示最边缘的面
                                    ...
                                    //显示前面(背向屏幕的面,镜头不支持转动的话跟侧面一样就算能显示也看不到)
                                    if (CheckAdjacent(x, y, Direction.forward, transparency) == true)
                                        CreateFace(cellId, Facing.forward, colliderType, x, y);
                                }

                                //横版模式只显示后面(玩家屏幕看到的面)
                                //if (CheckAdjacent(x, y, Direction.back, transparency)) 
                            ...
                            { // if not cube.若是自定义网格
                                ...
                                { // if any adjacent cellId isn'transform opaque, we render the mesh.若任何相邻体素不是不透明的,自定义渲染网格
        ...
        /// <returns>若一个面应该被创建时返回true</returns>
        ...
        { // returns true if a face should be spawned
            ...
            else
            {
                ...
                { //if the neighbor chunk is missing.若相邻团块是缺失状态或空块(CellID返回了65535)
                  //若设置需要显示边界面或者这个面的朝向是向上的
                ...
                {//执行此检查的体素块非完全透明(半透明或固体)
                    ...
                    {//若邻块非固体(透明或半透明),允许绘制执行此检查的体素块的检查面
        ...
        /// <param name="y"></param>
        /// <param name="z"></param>
        /// <returns>若所有相邻体素块都是实体,则返回true</returns>
        ...
        private void CreateFace(ushort cellId, Facing facing, ColliderType colliderType, int x, int y, int z)
        {
            ...
            else
            {
                ...
                //默认情况下,Mesh的索引缓冲区为16位,支持最多65536个顶点,若需要支持更多顶点,可以设置为32位,但需注意并非所有平台都支持32位索引
